...
import AbstractTensors: scalar, involute, unit, even, odd, vectorspace
...
@eval begin
    ...
    @pure function getalgebra(V::SubManifold{M,N,S}) where {M,N,S}
        ...
        getalgebra(ndims(M),DirectSum.options(M),metric(M),S,typeof(M),diffvars(M),DirectSum.diffmode(M))
    end
end
