...
As a result of this macro, all of the `SubManifold{V⊕V',G}` elements generated by that `VectorBundle` become available in the local workspace with the specified naming.
The first argument provides pseudoscalar specifications, the second argument is the variable name for the `Manifold`, and the third and fourth argument are variable prefixes of the `SubManifold` vector names (and covector basis names).
...
@inline Base.one(t::Type{T}) where T<:VectorBundle = one(t())
@inline Base.zero(t::Type{T}) where T<:VectorBundle = zero(t())
Base.one(V::Manifold) = SubManifold{V}()
